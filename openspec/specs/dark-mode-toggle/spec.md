...
Global page styles SHALL update when the theme changes. The body background, link color, and footer border SHALL switch between light and dark values based on the `dark-mode` class on `<html>`.
...
- **WHEN** dark mode is active
- **THEN** links render with a lighter, high-contrast color suitable for dark backgrounds
